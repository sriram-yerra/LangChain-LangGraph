import os
from dotenv import load_dotenv

# LangChain Vertex AI
from langchain_google_vertexai import ChatVertexAI

# LangChain components
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.output_parsers import StrOutputParser
from langchain_core.runnables import RunnableSequence


# ==============================
# Load environment variables
# ==============================
load_dotenv()

required_vars = [
    "GEMINI_MODEL",
    "TEMPERATURE",
    "GOOGLE_CLOUD_PROJECT",
    "GOOGLE_CLOUD_REGION",
    "MAX_OUTPUT_TOKENS"
]

for var in required_vars:
    if not os.getenv(var):
        raise ValueError(f"{var} not found in .env")

print("Vertex AI configuration loaded")

# ==============================
# TASK 1 — Prompt Template
# ==============================
prompt_template = ChatPromptTemplate.from_messages([
    ("system", "You are a helpful assistant"),
    ("human", "{input}")
])

# ==============================
# TASK 2 — Vertex AI LLM
# ==============================
llm_vertexAI = ChatVertexAI(
    model_name=os.getenv("GEMINI_MODEL"),
    temperature=float(os.getenv("TEMPERATURE")),
    project=os.getenv("GOOGLE_CLOUD_PROJECT"),
    location=os.getenv("GOOGLE_CLOUD_REGION"),
    max_output_tokens=int(os.getenv("MAX_OUTPUT_TOKENS")),
    convert_system_message_to_human=True
)

# ==============================
# TASK 3 — Output Parser
# ==============================
str_parser = StrOutputParser()

# ====================================================
# Chain Style-1: Manual Invocation
# ====================================================
template = prompt_template.invoke({"input": "What is the capital of France?"})
res = llm_vertexAI.invoke(template)
final_result = res.content
print("\nManual Invocation Result:")
print(final_result)

# ====================================================
# Chain Style-2: Chain Invocation using LCEL "|" Operatoe. (pipe operator)
# ====================================================
chain1 = prompt_template | llm_vertexAI | str_parser
result1 = chain1.invoke({"input": "What is the capital of France?"})
print("\nLCEL Chain Result:")
print(result1)

# ====================================================
# Chain Style-3: RunnableSequence Chain
# ====================================================
chain2 = RunnableSequence(
    prompt_template,
    llm_vertexAI,
    str_parser
)
result2 = chain2.invoke({"input": "What is the capital of France?"})
print("\nRunnableSequence Result:")
print(result2)